...
export default function VolunteerDashboard() {
  ...
  const handleUpdateStatus = async (postId: string, pickupId: string, status: string) => {
            ...
            {posts.map((post) => (
              <div key={post.id} className="card hover:shadow-lg transition-shadow">
                ...
                <div className="space-y-3">
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900">{post.foodType}</h3>
                    <p className="text-sm text-gray-600">by {post.hostName}</p>
                  </div>

                  <div className="flex items-center gap-2 text-gray-700">
                    <Users className="w-4 h-4" />
                    <span className="text-sm">{post.servings} servings ({post.quantity})</span>
                  </div>
                  ...
                  {post.status === 'POSTED' && filter === 'available' && new Date(post.expiresAt) >= new Date() && (
                    <button
                  ...
                  {post.status === 'POSTED' && filter === 'available' && new Date(post.expiresAt) < new Date() && (
                    <div className="w-full mt-4 px-4 py-2 bg-red-50 border border-red-200 rounded-lg text-center">
                      <span className="text-red-600 font-medium text-sm">This food has expired</span>
                    </div>
                  )}

                  {post.status === 'ACCEPTED' && filter === 'my-pickups' && (
                    <button
                      onClick={() => handleUpdateStatus(post.id, post.pickupId || '', 'PICKED')}
                      className="w-full btn-primary mt-4"
                    >
                      Mark as Picked Up
                    </button>
                  ...
                  {post.status === 'PICKED' && filter === 'my-pickups' && (
                    <button
                      onClick={() => handleUpdateStatus(post.id, post.pickupId || '', 'COMPLETED')}
